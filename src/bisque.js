...
(function(){
	...
	kit.knob = function( sel, trs ){
		$$( sel ).forEach( function( elem ){
			...
			elem.style.display = 'none';
			elem.parentNode.insertBefore( knobInput, elem.nextSibling );
		} );
	};
	...
	function getProps( el ){
		var p = {
			...
			start: isNum( el[ ATTR ]('rotate-start') ) ? mmax( +el[ ATTR ]('rotate-start'), -180 ) : -135,
			end: isNum( el[ ATTR ]('rotate-end') ) ? mmin( +el[ ATTR ]('rotate-end'), 180 ) : 135,
...
(function(){
	...
	function px2em( text ){
		var m, i, j, before, after, reg = /@-bis-px2em\s(\d+),/g;
	...
	parse = function( cssText ){
		...
		cssText = cssText.replace(/(@-bis-px2em\s\d+){/g, '$1,');
